        ...
        if(item_.getVenue().getPhotos().getGroups().size() > 0) {
           ...
           // viewHolder.tvComment.setText(image);
            ...
            if(item_.getVenue().getCategories().size() > 0){
                String prefixSufix = "";
                prefixSufix += item_.getVenue().getCategories().get(0).getIcon().getPrefix() + "256";
                ...
                Picasso.get().load(url1).into((ImageView)viewHolder.ivLogo);
                //Toast.makeText(getContext(), url1, Toast.LENGTH_SHORT).show();
